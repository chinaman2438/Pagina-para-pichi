...
products.forEach(p => {
    ...
    if (fs.existsSync(prodDir)) {
        ...
        files.forEach(file => {
            if (file.match(/\.(jpg|jpeg|png)$/i)) {
                // IMPORTANT: encodeURI the path so spaces don't break HTML attributes but slashes remain
                const imgPath = 'Products/' + encodeURIComponent(p.folder) + '/' + encodeURIComponent(file);
                // Also replacing %20 with space in alt tag is fine
                imagesHtml += '<div class="swiper-slide"><img src="' + imgPath + '" alt="' + p.name + '"></div>\n';
            }
        });
    } else {
        console.warn('Folder not found:', prodDir);
    }

    let descHtml = '<ul>';
    const descFile = path.join(prodDir, 'Description.md');
    if (fs.existsSync(descFile)) {
        const buf = fs.readFileSync(descFile);
        ...
        console.warn('Description not found:', descFile);
    }
    descHtml += '</ul>';

    if (!p.brochure || p.brochure === '#') {
        p.brochure = '#';
    }
    ...
    // Save with UTF8 explicitly
    fs.writeFileSync(p.id + '.html', html, 'utf8');
